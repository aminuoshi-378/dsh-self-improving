...
在 dsh Web Settings 贡献一个 **Rules** 标签页，编辑**一段软规则文本**（markdown），经 settings 传输存到后端 `~/.dsh/rules.md` 文件，并作为建议注入 agent 系统提示词。
...
pnpm pack                          # → dsh-rule-enforcement-gui-<version>.tgz（版本号取 package.json）
dsh plugin --profile web add D:\绝对路径\dsh-rule-enforcement-gui-<version>.tgz
```
