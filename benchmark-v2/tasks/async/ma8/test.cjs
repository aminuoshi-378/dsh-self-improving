...
;(async () => {
  const seq = [['fail',2],['fail',2],['ok',1],['fail',1],['fail',1],['fail',1]]
  const out = []
  for (const [kind, fails] of seq) {
    try {
      const v = await callEndpoint('svc-a2', async () => {
        if (kind === 'fail') { out.push('F'); throw new Error('endpoint down') }
        out.push('S')
        return 'ok-' + kind
      }, { retries: 1, baseMs: 1, threshold: 2, cooldownMs: 80 })
      out.push('R:' + v)
    } catch (e) {
  ...
  eqHash(JSON.stringify(out), '4c53cf4111961f29a3a464ca2395d0483515ed47737161b9911da66b55588dc5', 'breaker state machine trace')
