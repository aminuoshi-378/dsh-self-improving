...
;(async () => {
  const seq = [['ok',1],['fail',1],['fail',1],['fail',1],['ok',1]]
  const out = []
  for (const [kind, fails] of seq) {
    try {
      const v = await callEndpoint('svc-a0', async () => {
        if (kind === 'fail') { out.push('F'); throw new Error('endpoint down') }
        out.push('S')
        return 'ok-' + kind
      }, { retries: 1, baseMs: 1, threshold: 2, cooldownMs: 160 })
      out.push('R:' + v)
    } catch (e) {
  ...
  eqHash(JSON.stringify(out), 'b7e9aa579b30b115ce0ce18eeceaec5cb8d68e0c9fc4bd7b2b67b3540da6773d', 'breaker state machine trace')
