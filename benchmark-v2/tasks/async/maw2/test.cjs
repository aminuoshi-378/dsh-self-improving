...
;(async () => {
  const records = [{'id':1,'qty':5,'unitPrice':2},{'id':2,'qty':'x','unitPrice':1},{'id':3,'qty':2,'unitPrice':7},{'id':4,'qty':1,'unitPrice':1,'discount':1}]
  const etl = await runETL(records.map((r) => ({ ...r })), {'readConcurrency':1,'computeConcurrency':2})
  eqHash(JSON.stringify(etl), 'b236a3611beea93426231c51bb39893a4edd4feebabdf22513eee54e7aa43065', 'etl pipeline with failures dropping slots')
