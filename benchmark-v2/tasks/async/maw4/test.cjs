...
;(async () => {
  const records = []
  const etl = await runETL(records.map((r) => ({ ...r })), {'readConcurrency':2,'computeConcurrency':2})
  eqHash(JSON.stringify(etl), '68ca0fc3918e02405e48d465654ea7eee4ccf2ccdf9fade0b5d742863a29adc7', 'etl pipeline with failures dropping slots')
