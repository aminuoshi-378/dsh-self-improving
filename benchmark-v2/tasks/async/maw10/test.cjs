...
;(async () => {
  const seq = [['fail',3],['ok',2],['fail',1],['ok',1],['fail',1],['fail',1],['ok',1]]
  const out = []
  for (const [kind, fails] of seq) {
    try {
      const v = await callEndpoint('svc-a4', async () => {
        if (kind === 'fail') { out.push('F'); throw new Error('endpoint down') }
        out.push('S')
        return 'ok-' + kind
      }, { retries: 1, baseMs: 1, threshold: 2, cooldownMs: 200 })
      out.push('R:' + v)
    } catch (e) {
  ...
  eqHash(JSON.stringify(out), '007a967239bffce9eea50b4acdc12661a054d3500d0b38dcc5fe4e2df89e8ce7', 'breaker state machine trace')
