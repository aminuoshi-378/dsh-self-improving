...
;(async () => {
  const seq = [['fail',1],['fail',1],['fail',1],['fail',1],['fail',1]]
  const out = []
  for (const [kind, fails] of seq) {
    try {
      const v = await callEndpoint('svc-a1', async () => {
        if (kind === 'fail') { out.push('F'); throw new Error('endpoint down') }
        out.push('S')
        return 'ok-' + kind
      }, { retries: 1, baseMs: 1, threshold: 3, cooldownMs: 120 })
      out.push('R:' + v)
    } catch (e) {
  ...
  eqHash(JSON.stringify(out), 'fee67aac733ba6e7c07b09377bad28fc40e550de0e50b9ddf4f81d74074bcaca', 'breaker state machine trace')
