...
const lines = ['x','x','y','x','z','x']
const fs = new MemFS()
const deds = []
for (const l of lines) deds.push(appendDedup(fs, 'raw.txt', l, 3))
eqHash(JSON.stringify({ deds, raw: fs.readFile('raw.txt') }), '32f79a440faf8efd1b5b1953176057f9c426c088c7b65ca8900ba082782dc838', 'trailing-window dedup')
const fs2 = new MemFS()
...
for (const l of ['r1','r2','r3','r4','r5','r1','r6','r7','r8']) {
  logResults.push([l, appendLog(fs3, 'svc', { maxLines: 2, keep: 2, window: 3 }, l)])
}
